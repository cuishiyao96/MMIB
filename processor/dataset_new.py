...
class MMPNERDataset(Dataset):
    def __init__(self, processor, transform, img_path=None, aux_img_path=None, max_seq=40, sample_ratio=1, mode='train', ignore_idx=0) -> None:
        ...
        self.aux_img_path = aux_img_path  if aux_img_path is not None else None
        self.mode = mode
    ...
    def __getitem__(self, idx):
        word_list, label_list, img, aux_imgs = self.data_dict['words'][idx], self.data_dict['targets'][idx], self.data_dict['imgs'][idx], self.data_dict['aux_imgs'][idx]
        tokens, labels = [], []
        ...
        if self.img_path is not None:
            ...
            if self.aux_img_path is not None:
                ...
                for aux_img_name in aux_imgs:
                    aux_img_paths.append( os.path.join(os.path.join(self.aux_img_path, img_name), aux_img_name) )

                for i in range(len(aux_img_paths)):
                ...
                assert len(aux_images) == 4
                return torch.tensor(input_ids), torch.tensor(token_type_ids), torch.tensor(attention_mask), torch.tensor(labels), torch.tensor(image), torch.tensor(aux_images)
